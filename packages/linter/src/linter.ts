...
export function lintDesignMd(doc: ParsedDoc): LintReport {
  ...
  for (const ref of tokenRefs) {
    ...
    if (val === undefined) {
      addFinding(
        ...
        "broken-ref",
        inner,
        `Token reference ${ref} does not resolve to any defined token.`
      );
    }
  }
      ...
      if (bg && fg && isHex(bg) && isHex(fg)) {
        ...
        if (ratio !== null && ratio < 4.5) {
          addFinding(
            ...
            "contrast-ratio",
            `components.${name}`,
            `textColor (${fg}) on backgroundColor (${bg}) has contrast ratio ${ratio.toFixed(
              2
  ...
  if (
    ...
    for (const colorKey of Object.keys(tokens.colors)) {
      ...
      const directRef = Object.values(tokens.components).some(
        (c) => c && Object.values(c).some((v) => v === refStr)
      );
      if (!directRef) {
        addFinding(
          "info",
          "orphaned-tokens",
          `colors.${colorKey}`,
          `Color token "${colorKey}" is defined but never referenced by any component.`
        );
      }
    }
  }
  ...
  if (!tokens.spacing || Object.keys(tokens.spacing).length === 0) {
    addFinding(
      ...
      "missing-sections",
      "spacing",
      "No spacing scale defined. Consider adding sm/md/lg tokens for consistent layout."
    );
  }
  ...
  if (!tokens.rounded || Object.keys(tokens.rounded).length === 0) {
    addFinding(
      ...
      "missing-sections",
      "rounded",
      "No border-radius tokens defined."
    );
  }
    ...
    for (const h of headings) {
      ...
      if (idx !== -1 && idx < lastIdx) {
        addFinding(
          ...
          "section-order",
          "sections",
          `Section "${h}" appears out of canonical order. Expected: Overview → Colors → Typography → Layout → Components.`
        );
        break;
      }
